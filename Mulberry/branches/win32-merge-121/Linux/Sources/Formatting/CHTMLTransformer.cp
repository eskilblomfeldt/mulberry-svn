...
char* CHTMLUtils::ConvertToEnriched(const char* htmlText)
{
	...
	switch(CPreferences::sPrefs->mHTMLTextFontInfo.GetValue().size)
	{
	case 9:
		CurrentSize = 2;
		break;
...
CHTMLTransformer::CHTMLTransformer(CFormattedTextDisplay* display)
{
	mText = display;
...
char* CHTMLTransformer::Transform()
{
	...
	for(long i = 0; i < length; i++)
	{
		...
		if(i >= runExpires)
		{
			...
			runExpires += mText->GetStyles().GetRunLength(runNumber);
			ExamineStack(mText->GetStyles().GetRunDataRef(runNumber));
			runNumber++;
...
void CHTMLTransformer::ExamineStack(const JTextEditor16::Font& wRunInfo)
{																	
	...
	if ((wRunInfo.align == JTextEditor16::kAlignCenter) && !aligning)
	{
	...
	case E_FONT:
		{
			...
			if (myFonting || mySizing || myColoring)
			{
				...
				if (myColoring)
				{
					...
					mOut << " color=\"#";
					::snprintf(myColor, 8, "%.2x%.2x%.2x", felement->GetRed() >> 8, felement->GetGreen() >> 8, felement->GetBlue() >> 8);
					mOut << myColor;
					mOut << "\"";
				}
