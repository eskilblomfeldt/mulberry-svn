...
bool CActionManager::MDNMessage(CMessage* msg)
{
	...
	// Get addresses
	auto_ptr<CAddressList> to_list(new CAddressList);
	auto_ptr<CAddressList> cc_list(new CAddressList);
	auto_ptr<CAddressList> bcc_list(new CAddressList);
	...
	// Create body
	auto_ptr<CAttachment> body(msg->CreateMDNSeenBody(id, automatic));
...

	{
		...
		auto_ptr<CMessage> msg(CreateMessage(to_list.release(), cc_list.release(), bcc_list.release(), subject, body.get(), id, NULL, NULL, NULL));
...
bool CActionManager::NewDraftiTIPAutomatic(const CAddressList* to, const CAddressList* cc, const CAddressList* bcc, const cdstring& subject, const cdstring& description, CDataAttachment* attach, const CIdentity* id, CMessage* msg)
{
	...
	
	{
		...
		// Create the message (it takes possesion of address lists)
		auto_ptr<CMessage> rmsg(CreateMessage((to != NULL) ? new CAddressList(*to) : NULL, 
									...
									const cdstring& text, const cdstring& identity, bool tied)
{
	...
	// Create body
	auto_ptr<CAttachment> body(new CDataAttachment(body_txt.grab_c_str()));

	{
		// Create the message (it takes possesion of address lists)
		auto_ptr<CMessage> msg(CreateMessage(to_list.release(), cc_list.release(), bcc_list.release(), subject, body.get(), id, &id->GetDSN(true), &msgs, NULL));
										...
										const cdstring& text, const cdstring& identity, bool tied)
{
	...
	// Get addresses
	auto_ptr<CAddressList> to_list(new CAddressList(addresses.mTo, addresses.mTo.length()));
	auto_ptr<CAddressList> cc_list(new CAddressList(addresses.mCC, addresses.mCC.length()));
	auto_ptr<CAddressList> bcc_list(new CAddressList(addresses.mBcc, addresses.mBcc.length()));
...
bool CActionManager::BounceMessages(CMessageList& msgs, NMessage::SAddressing addresses, const cdstring& identity, bool tied)
{
	...
	auto_ptr<char> bounce(::strdup(msgs.front()->GetHeader()));

	{
		// Create the message (it takes possesion of address lists)
		auto_ptr<CMessage> msg(CreateMessage(to_list.release(), cc_list.release(), bcc_list.release(), subject, body.get(), id, &id->GetDSN(true), &msgs, bounce.get()));
...
bool CActionManager::RejectMessages(CMessageList& msgs, bool return_msg, const cdstring& identity, bool tied)
{
	...
	// Create body
	auto_ptr<CAttachment> body(msgs.front()->CreateRejectDSNBody(return_msg));
										...
										cdstring& text)
{
...

	{
		...
		{
			auto_ptr<const char> quoted(CTextEngine::QuoteLines(plain_text, plain_text.length(),
