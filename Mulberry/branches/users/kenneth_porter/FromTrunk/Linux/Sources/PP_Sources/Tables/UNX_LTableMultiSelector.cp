	...
	const JXKeyModifiers& modifiers)
{
	...
	ClickSelect(inCell, modifiers);	// First handle as a normal click
	...
	TableIndexT		inFromRow)
{
	...
	size_t sel_count = std::count(mSelection.begin() + GetSelectionIndex(start), mSelection.begin() + GetSelectionIndex(stop) + 1, true);
	...
	if (!mFirstSelection.IsNullCell())
	{
		...
		else
		{
			...
			{
				// Find next true item
				std::vector<bool>::const_iterator found = std::find(mSelection.begin() + GetSelectionIndex(STableCell(inFromRow, 1)), mSelection.end(), true);
			...
			{
				// Find next true item
				std::vector<bool>::reverse_iterator found = std::find(mSelection.rbegin(), mSelection.rend(), true);
	...
	TableIndexT		inFromCol)
{
	...
	if (!mFirstSelection.IsNullCell())
	{
		...
		else
		{
			...
			{
				// Find next true item
				std::vector<bool>::const_iterator found = std::find(mSelection.begin(), mSelection.end(), true);
				if (found != mSelection.end())
				{
					std::vector<bool>::size_type index = found - mSelection.begin();
					mFirstSelection = STableCell(index/cols + 1, (index % cols) + 1);
