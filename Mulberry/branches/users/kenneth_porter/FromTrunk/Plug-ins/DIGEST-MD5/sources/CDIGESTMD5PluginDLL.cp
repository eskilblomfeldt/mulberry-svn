...
long CDIGESTMD5PluginDLL::ProcessFirstData(SAuthPluginData* info)
{
	...
	bool got_qop = false;
	auto_ptr<const char> nonce;
	auto_ptr<const char> opaque;
	auto_ptr<const char> realm;
	auto_ptr<const char> cnonce;
	...
	char hex_h_a1[33];
	{
		auto_ptr<char> a1_data(new char[::strlen(userid.get()) +
	...
	auto_ptr<char> digest_uri_value(new char[
										4 + // serv-type
										1 + // /
										::strlen(mServer) + // host
										1]);
	...
	{
		auto_ptr<char> a2_data(new char[
									12 + // AUTHENTICATE:
	...
	{
		char* kd1 = hex_h_a1;
		auto_ptr<char> kd2(new char[::strlen(nonce.get()) +
								::strlen(cnonce.get()) +
								4 + // :'s
								8 + // nc_value
								4 + // "auth"
