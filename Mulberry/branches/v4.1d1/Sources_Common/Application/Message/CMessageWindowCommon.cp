...
// Server logs on/off
void CMessageWindow::ServerState(bool logon)
{
#if 0
	if (logon)
	{
...
void CMessageWindow::Text2UTF16(i18n::ECharsetCode charset)
{
...
// Reset message text
void CMessageWindow::ShowPart(CAttachment* attach)
{
...
void CMessageWindow::ExtractPart(CAttachment* part, bool view)
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
CMessageWindow* CMessageWindow::FindWindow(const CMessage* theMsg, bool owned_by)
{
	// Does window already exist?
	cdmutexprotect<CMessageWindowList>::lock _lock(sMsgWindows);
	for(CMessageWindowList::iterator iter = sMsgWindows->begin(); iter != sMsgWindows->end(); iter++)
	{
...
// Create a new draft
void CMessageWindow::DoNewLetter(bool option_key)
{
...
void CMessageWindow::OnEditSpellCheck()
{
...
// Speak message
#ifdef __use_speech
void CMessageWindow::SpeakMessage()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::OnMessageReadPrev()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
// Read next message
void CMessageWindow::OnMessageReadNext()
{
	ReadNextMessage(false, false, false);
}

// Read next message
void CMessageWindow::ReadNextMessage(bool delete_it, bool copy_it, bool option_key)
{
	// Only do if message exists
	if (!mItsMsg)
		return;
	...
	if (!mItsMsg->IsSubMessage())
	{
		...
		while(!done)
		{
			...
			else
			{
				...
				{
					// Close if rollover not requested
...
void CMessageWindow::OnMessageDeleteRead()
{
	ReadNextMessage(true, false, false);
}

// Delete this then read next message
void CMessageWindow::CopyReadNextMessage(bool option_key)
{
	ReadNextMessage(false, true, option_key);
}

// Display message in existing window
bool CMessageWindow::RedisplayMessage(CMessage* theMsg)
{
	...
	CMessageWindow* other = FindWindow(theMsg);
	if (other)
	{
...
void CMessageWindow::DoRollover(CMbox* current_mbox)
{
	...
	else
	{
		// Show it again
#if __dest_os == __mac_os || __dest_os == __mac_os_x
		FRAMEWORK_WINDOW_TO_TOP(this)
#elif __dest_os == __win32_os
		GetParentFrame()->ShowWindow(SW_SHOW);
#elif __dest_os == __linux_os
		GetWindow()->GetDisplay()->Synchronize();
		GetWindow()->Show();
#endif
...
void CMessageWindow::ReplyToThisMessage(EReplyTo reply_to, bool option_key)
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::ForwardThisMessage(bool option_key)
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
// Bounce this message
void CMessageWindow::OnMessageBounce()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
// Reject this message
void CMessageWindow::OnMessageReject()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::OnMessageSendAgain()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
bool CMessageWindow::CopyThisMessage(CMbox* mbox, bool option_key)
{
	// Only do if message exists
	if (!mItsMsg)
		return false;
...
void CMessageWindow::OnMessageFlags(NMessage::EFlags flag)
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::OnMessageViewCurrent()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::OnMessageDecodeBinHex()
{
	if (mShowText)
	{
...
void CMessageWindow::OnMessageDecodeUU()
{
	if (mShowText)
	{
		// Create data stream to hold message text
...
// Verify/decrypt embedded message
void CMessageWindow::OnMessageVerifyDecrypt()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
...
void CMessageWindow::OnMessageDelete()
{
	// Only do if message exists
	if (!mItsMsg)
		return;
	...
	if (mItsMsg->IsDeleted())
	{
		// Undelete message
		mItsMsg->ChangeFlags(NMessage::eDeleted, false);
		MessageChanged();
	}
	else
	{
...
void CMessageWindow::OnViewAs(EView view)
{
...
void CMessageWindow::OnIncreaseFont()
{
...
void CMessageWindow::OnDecreaseFont()
{
...
void CMessageWindow::OnQuoteDepth(EQuoteDepth quote)
{
	long new_depth = 0;
...
// Read in raw body
bool CMessageWindow::ShowRawBody()
{
