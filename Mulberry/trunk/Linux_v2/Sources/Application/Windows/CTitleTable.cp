...
int CTitleTable::sClickColumn = 0;
...
void CTitleTable::OnCreate()
{
	// Get window that owns this one
	mTableWindow = dynamic_cast<CTableWindow*>(GetWindow()->GetDirector());
...
void CTitleTable::SetTitleInfo(bool can_sort, bool can_size, const char* rsrcid, unsigned long max_str, const cdstring& menu_items)
{
	mCanSort = can_sort;
	mCanSize = can_size;
...
void CTitleTable::HandleMouseDown(const JPoint& pt, const JXMouseButton button,
																	...
																	const JXKeyModifiers& modifiers)
{
	...
	if (!InDragRegion(pt, cell) && mCanSort && (clickCount == 1) && button == kJXLeftButton)
	{
		...
		if (!sort_col)
			// Change sort by
			mTableWindow->SetSortBy(GetColumns()[cell.col - 1].column_type);
		else
	...
	)
{
	if (mDragType != kInvalidDrag)
	{
		...
		mTableWindow->SetColumnWidth(mDragCell.col, colWidth);
	}
...
void CTitleTable::OnRButtonDown(const JPoint& point,
								const JXButtonStates& buttonStates,
								const JXKeyModifiers& modifiers)
{
	// reset menu titles
...
void CTitleTable::Receive(JBroadcaster* sender, const Message& message)
{
...
void CTitleTable::AdjustScrollbars()
{
	// Ignore - do not do scrollbar state changes - the table (not titles) controls the scroll range
}

void CTitleTable::DrawBorder(JXWindowPainter& p, const JRect& frame)
{
...
void CTitleTable::DrawBackground(JXWindowPainter& p, const JRect& frame)
{
...
void CTitleTable::DrawCell(JPainter* p, const STableCell& inCell, const JRect &rect)
{
	StPenState savestate(p);
...
void CTitleTable::DrawItem(JPainter* p, SColumnInfo& col_info, const JRect &cellRect)
{
...
// Draw icon
void CTitleTable::DrawIcon(JPainter* p, SColumnInfo& col_info, unsigned int icon_id, const JRect &cellRect)
{
	// Get icon from cache
