...
namespace _GLIBCXX_STD
{
  template<typename _Tp, typename _Alloc>
	...
	__throw_length_error(__N("svector::reserve"));
  ...
  template<typename _Tp, typename _Alloc>
    typename svector<_Tp, _Alloc>::iterator
    svector<_Tp, _Alloc>::
  ...
  template<typename _Tp, typename _Alloc>
    svector<_Tp, _Alloc>&
    svector<_Tp, _Alloc>::
    operator=(const svector<_Tp, _Alloc>& __x)
    {
      if (&__x != this)
	{
  ...
  template<typename _Tp, typename _Alloc>
    ...
    _M_fill_assign(size_t __n, const value_type& __val)
    {
      if (__n > capacity())
	{
	  svector __tmp(__n, __val, _M_get_Tp_allocator());
	  __tmp.swap(*this);
	}
      else if (__n > size())
	{
  ...
  template<typename _Tp, typename _Alloc>
      ...
      else
	{
	  ...
	  if (__old_size == this->max_size())
	    __throw_length_error(__N("svector::_M_insert_aux"));
	  ...
	  if (keep_small)
	    ...
	    __len = __old_size != 0 ? 2 * __old_size : 1;	  
	  if (__len < __old_size)
	    __len = this->max_size();
  ...
  template<typename _Tp, typename _Alloc>
      ...
      if (__n != 0)
	{
	  if (size_type(this->_M_impl._M_end_of_storage
		...
		__throw_length_error(__N("svector::_M_fill_insert"));
		...
		__len = __old_size + std::max(__old_size, __n);
	      if (__len < __old_size)
		__len = this->max_size();
	...
	if (__first != __last)
	  {
		...
		const size_type __old_size = size();
		if (this->max_size() - __old_size < __n)
		  __throw_length_error(__N("svector::_M_range_insert"));	
		...
		if (keep_small)
		  ...
		  __old_size + std::max(__old_size, __n);
		if (__len < __old_size)
		  __len = this->max_size();
